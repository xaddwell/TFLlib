...
def run_experiment(args):
    ...
    while True:
        command = [
            ...
            "--privacy_attack", args["reconstruction_attack_types"],
            "--cid", "0",
            "--global_rounds", str(args['global_rounds']),
        ]
...
if __name__ == "__main__":
    ...
    num_clients = [10, 100]
    global_rounds = [2]
    reconstruction_attack_types = ['dlg', 'see_thru_grads', 'RobFed', 'LOKI'] #'inv_grads'
    datasets = ['CIFAR10', 'FEMNIST']
    combinations = list(itertools.product(batch_sizes, local_epochs, num_clients, reconstruction_attack_types, datasets, global_rounds))
    
    for batch_size, local_epoch, num_client, reconstruction_attack_type, dataset, global_round in combinations:
        base_cfg = get_base_cfg(dataset)
        base_cfg["exp_name"] = args.exp_name
        base_cfg["reconstruction_attack_types"] = reconstruction_attack_type
        ...
        task_id = f"{dataset}_{reconstruction_attack_type}_bz{batch_size}_local_epoch-{local_epoch}_num_clients-{num_client}_gloval_round-{global_round}_{hetero_type}"
        if os.path.exists(os.path.join(f"/home/zzm/RFLlib/log/{args.exp_name}", f"{task_id}")):
            continue
