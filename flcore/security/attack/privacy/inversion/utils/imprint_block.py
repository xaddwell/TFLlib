...
class SparseImprintLayer(torch.nn.Module):
    def __init__(self, number_bins, cid, data_ch=3, im_dim=[32,32], conv_scale=500, conv_sizes=0, mean=0., std=1., linfunc = 'avg'):
        super().__init__()
        self.myvalue = data_ch * im_dim[0] * im_dim[1]
        self.data_shape = (data_ch, *im_dim)
        self.conv_1 = torch.nn.Conv2d(in_channels=data_ch, out_channels=conv_sizes, kernel_size=(3,3), stride=1, padding='same')
        self.act = torch.nn.Hardtanh(min_val=0, max_val=1)
        self.FC1 = torch.nn.Linear(conv_sizes*im_dim[0]*im_dim[1], number_bins)
        self.FC2 = torch.nn.Linear(number_bins, self.myvalue)

        self.num_bins = number_bins
        self.mean, self.std = mean, std
        self._init_conv_function(cid, data_ch, conv_scale, linfunc)
    ...
    def _init_conv_function(self, cid, data_ch, conv_scale, linfunc):
        init_biases, bin_width = self._get_bins(linfunc)
        with torch.no_grad():
            self.FC1.weight = torch.nn.Parameter((1/conv_scale)*torch.ones(self.FC1.weight.size())/(self.myvalue)/torch.tensor(bin_width).unsqueeze(1))
            self.FC1.bias   = torch.nn.Parameter(-torch.tensor(init_biases, dtype=torch.float32)/torch.tensor(bin_width, dtype=torch.float32))    # FC1.bias[0].item() != init_biases[0], some minor precision loss
            single_client_conv1_weight = torch.zeros(self.conv_1.weight.size())
            for j in range(data_ch):
                single_client_conv1_weight[j + cid*data_ch, j, 1, 1] = 1 * conv_scale
            self.conv_1.weight = torch.nn.Parameter(single_client_conv1_weight)
            self.conv_1.bias = torch.nn.Parameter(torch.zeros(self.conv_1.bias.size()))
            self.FC2.weight = torch.nn.Parameter(1000*torch.ones(self.FC2.weight.size()))
            self.FC2.bias   = torch.nn.Parameter(torch.zeros(self.FC2.bias.size()))           
    ...
    def _get_bins(self, linfunc = 'avg'):
        ...
        for path in range(self.num_bins + 1):
            ...
            if "fourier" in linfunc:
                quantile = laplace(loc=sum(self.mean) / len(self.mean), scale=sum(self.std) / len(self.std) / math.sqrt(2)).ppf(mass)
            else:
                distribution = norm(loc=sum(self.mean) / len(self.mean), scale=sum(self.std) / len(self.std))
                quantile = distribution.ppf(mass)
            bins += [quantile]
            #bins += [NormalDist(mu=self.mean, sigma=self.std).inv_cdf(mass)]
        bins = torch.sort(torch.tensor(np.array(bins)))[0]    
        bin_sizes = [(bins[i + 1] - bins[i]).tolist() for i in range(len(bins) - 1)]
...
class OneShotBlockSparse(SparseImprintBlock):
    ...
    def _get_bins(self):
        ...
        bins = []
        mass_per_bin = 1 / self.num_bins
        inv_cdf = -norm.ppf(0.5)
        inv_cdf_mass = -norm.ppf(0.5 + mass_per_bin)
        bins = [inv_cdf, inv_cdf_mass]
