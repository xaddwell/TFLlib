...
class ShokriAttacker(MembershipInferenceAttack):
    ...
    def train(self, model, traindata):
        ...
        for i in range(self.epochs):
            batch_loss = []   
            for batched_x, batched_y in DataLoader(traindata, batch_size=self.config['train']['batch_size'], shuffle=True):
    ...
    def test(self, model, testdata):
        ...
        correct = 0.
        with torch.no_grad():
            for (batched_x, batched_y) in DataLoader(testdata, batch_size=self.config['train']['batch_size']):
                x = batched_x.to(self.device)
                y = batched_y.to(self.device)
                log_probs = model(x)
                self.outputs.append(log_probs.cpu().detach().numpy())
                self.classes.append(y.cpu().detach().numpy())
